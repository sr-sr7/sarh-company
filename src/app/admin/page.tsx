        ...
        {tab==='add' && (can('add_property')||(!!editId&&can('edit_property'))) && (
          <form onSubmit={saveProperty} style={S.form}>
            ...
            <div style={S.formGrid}>
              ...
              <div><label style={S.label}>وحدة السعر</label><select value={form.price_unit} onChange={e=>ff('price_unit',e.target.value)} style={S.select}>{['ريال','ريال / سنة','ريال / شهر','ريال / يوم','آخر سوم'].map(u=><option key={u}>{u}</option>)}</select></div>
              <div><label style={S.label}>{form.price_unit === 'آخر سوم' ? 'آخر سومة (اختياري)' : 'السعر *'}</label><input required={form.price_unit !== 'آخر سوم'} type="number" value={form.price} onChange={e=>ff('price',e.target.value)} style={S.input} placeholder={form.price_unit === 'آخر سوم' ? 'اتركه فارغاً إذا لم يتحدد' : '850000'} /></div>
