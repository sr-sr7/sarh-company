...
// ─── Styles ───────────────────────────────────────────────────
const S = {
  page:  { minHeight:'100vh', background:'#f4ede4', fontFamily:"'Tajawal','Cairo',sans-serif", direction:'rtl' as const },
  header:{ background:'#f4ede4', color:'#1e3a34', padding:'16px 32px', display:'flex', alignItems:'center', justifyContent:'space-between', borderBottom:'1px solid rgba(30,58,52,0.12)', flexWrap:'wrap' as const, gap:12 },
  ...
  statCard:{ background:'#f4ede4', color:'#1e3a34', borderRadius:16, padding:'20px 24px', display:'flex', alignItems:'center', gap:16, border:'1px solid rgba(30,58,52,0.1)' },
  ...
  tabBtn:(a:boolean)=>({ padding:'10px 20px', borderRadius:10, fontSize:'0.85rem', fontWeight:700, cursor:'pointer', border:a?'1px solid rgba(30,58,52,0.2)':'1px solid rgba(30,58,52,0.12)', background:a?'#f4ede4':'#fff', color:'#1e3a34', fontFamily:"'Tajawal','Cairo',sans-serif" }),
  msg:{ margin:'0 32px 16px', background:'#f4ede4', color:'#1e3a34', padding:'12px 16px', borderRadius:10, fontSize:'0.85rem', fontWeight:600 },
  ...
  input:{ width:'100%', background:'#f4ede4', border:'1px solid rgba(39,66,62,0.15)', borderRadius:10, padding:'10px 14px', fontSize:'0.9rem', outline:'none', fontFamily:"'Tajawal','Cairo',sans-serif", boxSizing:'border-box' as const },
  textarea:{ width:'100%', background:'#f4ede4', border:'1px solid rgba(39,66,62,0.15)', borderRadius:10, padding:'10px 14px', fontSize:'0.9rem', outline:'none', fontFamily:"'Tajawal','Cairo',sans-serif", resize:'vertical' as const, boxSizing:'border-box' as const },
  select:{ width:'100%', background:'#f4ede4', border:'1px solid rgba(39,66,62,0.15)', borderRadius:10, padding:'10px 14px', fontSize:'0.9rem', outline:'none', fontFamily:"'Tajawal','Cairo',sans-serif", boxSizing:'border-box' as const },
  ...
  uploadZone:{ border:'2px dashed rgba(39,66,62,0.25)', borderRadius:14, padding:24, textAlign:'center' as const, cursor:'pointer', background:'#f4ede4' },
  ...
  uploadBtn:{ display:'inline-block', marginTop:12, background:'#f4ede4', color:'#1e3a34', border:'1px solid rgba(30,58,52,0.2)', borderRadius:8, padding:'10px 20px', fontSize:'0.85rem', fontWeight:700, cursor:'pointer', fontFamily:"'Tajawal','Cairo',sans-serif" },
  ...
  submitBtn:{ background:'#f4ede4', color:'#1e3a34', padding:'12px 32px', borderRadius:10, fontWeight:700, fontSize:'0.95rem', border:'1px solid rgba(30,58,52,0.2)', cursor:'pointer', fontFamily:"'Tajawal','Cairo',sans-serif" },
...
export default function AdminPage() {
  ...
  if (!session) return (
    <div style={{ minHeight:'100vh', background:'#1e3a34', display:'flex', alignItems:'center', justifyContent:'center' }}>
      <div style={{ color:'#f4ede4', fontFamily:'Tajawal' }}>جاري التحقق...</div>
    </div>
  )
        ...
        {tab==='properties' && (
          ...
          <div style={{ marginBottom:16, position:'relative' }}>
            ...
            <input
              ...
              style={{ width:'100%', padding:'11px 40px 11px 14px', borderRadius:12, border:'1.5px solid rgba(39,66,62,0.18)', fontSize:'0.9rem', fontFamily:"'Tajawal','Cairo',sans-serif", outline:'none', background:'#f4ede4', boxSizing:'border-box' as const, direction:'rtl' }}
            />
          </div>
                ...
                : properties.map(p=>(
                  <tr key={p.id} data-pid={p.id} data-search={[p.title,p.district,p.city,p.type,String(p.listing_number||'')].join(' ').toLowerCase()}>
                    <td style={{...S.td,width:64}}>
                      {p.listing_number ? <span style={{background:'#f4ede4',color:'#1e3a34',fontSize:'0.72rem',fontWeight:800,padding:'3px 8px',borderRadius:6,fontFamily:'monospace'}}>#{p.listing_number}</span> : <span style={{color:'#ccc',fontSize:'0.72rem'}}>—</span>}
                    </td>
                    <td style={S.td}>
                      {p.main_image ? <img src={p.main_image} alt="" style={{width:56,height:44,objectFit:'cover',borderRadius:8}} />
                        : <div style={{width:56,height:44,background:'#f4ede4',borderRadius:8,display:'flex',alignItems:'center',justifyContent:'center',fontSize:'1.2rem'}}>🏠</div>}
                    </td>
          ...
          <div>
            <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:16}}>
              ...
              <a href={`data:text/csv;charset=utf-8,﻿الاسم,رقم الهاتف,نوع العقار,تاريخ التسجيل\n${promoLeads.map(l=>`${l.name},${l.phone},${l.property_type||''},${new Date(l.created_at).toLocaleDateString('ar-SA')}`).join('\n')}`} download="promo_leads.csv"
                style={{background:'#f4ede4',color:'#1e3a34',padding:'8px 18px',borderRadius:10,fontSize:'0.8rem',fontWeight:700,textDecoration:'none'}}>⬇️ تصدير CSV</a>
            </div>
            <div style={S.tableWrap}>
              <table style={S.table}>
        ...
        {tab==='add' && (can('add_property')||(!!editId&&can('edit_property'))) && (
          <form onSubmit={saveProperty} style={S.form}>
            ...
            <div style={S.formGrid}>
              ...
              <div><label style={S.label}>نوع العقار *</label><select required value={form.type} onChange={e=>ff('type',e.target.value)} style={S.select}>{['فيلا','أرض','شقة','استراحة','شاليه','دبلكس','محل تجاري','مستودع','مزرعة','قصر','وحدة علوية','وحدة أرضية','دور أرضي','دور علوي'].map(t=><option key={t}>{t}</option>)}</select></div>
                ...
                {form.map_url && (()=>{
                  ...
                  return <div style={{marginTop:12,borderRadius:12,overflow:'hidden',border:'2px solid rgba(39,66,62,0.15)'}}>
                    <div style={{background:'#27423e',padding:'8px 14px',display:'flex',alignItems:'center',justifyContent:'space-between'}}>
                      <span style={{color:'#f4ede4',fontSize:'0.75rem',fontWeight:700}}>📍 {lat.toFixed(5)}, {lng.toFixed(5)}</span>
                  ...
                  {editingUser.role==='user' && (
                    <div style={{marginBottom:24}}>
                      ...
                      <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(220px,1fr))',gap:10}}>
                        {PERMS.map(([key,label])=>(
                          <label key={key} style={{display:'flex',alignItems:'center',gap:10,padding:'10px 14px',borderRadius:10,border:`1px solid ${editingUser.permissions?.[key]?'#27423e':'rgba(39,66,62,0.15)'}`,background:editingUser.permissions?.[key]?'rgba(39,66,62,0.06)':'#f4ede4',cursor:'pointer',transition:'all 0.2s'}}>
              ...
              <form onSubmit={addUser}>
                ...
                {newUser.role==='user' && (
                  <div style={{marginBottom:20}}>
                    ...
                    <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(220px,1fr))',gap:10}}>
                      {PERMS.map(([key,label])=>(
                        <label key={key} style={{display:'flex',alignItems:'center',gap:10,padding:'10px 14px',borderRadius:10,border:`1px solid ${newUser.permissions[key]?'#27423e':'rgba(39,66,62,0.15)'}`,background:newUser.permissions[key]?'rgba(39,66,62,0.06)':'#f4ede4',cursor:'pointer',transition:'all 0.2s'}}>
