        ...
        {tab==='add' && (can('add_property')||(!!editId&&can('edit_property'))) && (
          <form onSubmit={saveProperty} style={S.form}>
            ...
            <div style={S.formGrid}>
              ...
              <div><label style={S.label}>السعر *</label><input required type="number" value={form.price} onChange={e=>ff('price',e.target.value)} style={S.input} placeholder="850000" /></div>
              <div><label style={S.label}>وحدة السعر</label><select value={form.price_unit} onChange={e=>ff('price_unit',e.target.value)} style={S.select}>{['ريال','ريال / سنة','ريال / شهر','ريال / يوم','أعلى سوم'].map(u=><option key={u}>{u}</option>)}</select></div>
