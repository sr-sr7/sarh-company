...
export default function PropertiesGrid() {
  ...
  return (
    <section id="properties" className="container-x py-14 md:py-20">
      ...
      {loading ? (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6">{[...Array(6)].map((_, i) => (<div key={i} className="bg-white rounded-2xl h-80 border border-ink/10 animate-pulse" />))}</div>
      ) : items.length === 0 ? (
        <div className="text-center py-20 text-moss-500"><p className="text-sm">لا توجد عقارات حالياً</p><a href="/admin" className="mt-4 inline-block bg-ink text-sand px-6 py-2 rounded-md text-sm font-medium">لوحة التحكم</a></div>
      ) : (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6">{items.map(p => (<PropertyCard key={p.id} property={p} />))}</div>
      )}
    </section>
  )
}
