'use client'import { useState, useEffect } from 'react'import Logo from './Logo'export default function Navbar() {  const [open, setOpen] = useState(false)  const [scrolled, setScrolled] = useState(false)  useEffect(() => {    const onScroll = () => setScrolled(window.scrollY > 8)    window.addEventListener('scroll', onScroll)    return () => window.removeEventListener('scroll', onScroll)  }, [])  const links = [    { href: '#properties', label: 'العقارات' },    { href: '#categories', label: 'الفئات' },    { href: '#services', label: 'خدماتنا' },    { href: '#contact', label: 'تواصل' },  ]  return (    <header      className={`sticky top-0 z-40 transition-all ${scrolled ? 'border-b border-ink/10' : ''}`}      style={{ background: scrolled ? 'rgba(250,248,245,0.85)' : 'transparent', backdropFilter: scrolled ? 'blur(12px)' : 'none' }}    >      <div className="container-x flex items-center justify-between h-16">        <a href="/" className="flex items-center gap-2">          <Logo size={36} />          <div className="leading-none">            <div className="font-amiri text-lg text-ink font-medium">صرح</div>            <div className="text-[8px] text-moss-500 tracking-[2px] mt-0.5">SARH REAL ESTATE</div>          </div>        </a>        <nav className="hidden md:flex items-center gap-7">'use client'
